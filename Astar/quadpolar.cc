#include "quadpolar.h"
#include "model.h"
QuadPolar::QuadPolar(Model &model)
  : model_(model){
  //if(model_.dimen()!=2){
...
double QuadPolar::Integrate(double * xmin, int nmin, double *xmax, int nmax){
...
double QuadPolar::Integrate1D_(double x1, double x2){
  //x2>x1
  double xm = 0.5*(x1+x2);
  double xr = 0.5*(x2-x1);
  double s = 0;
...
double QuadPolar::Integrate2D_(double x1, double x2, double y){
  ...
  for (int j=0; j<5; j++){
    ...
    xm1[1] = xm+dx;
    xm2[1] = xm-dx;
    s+=w_[j]*(xm1[0]*model_.fx(xm1,kdimen_) + xm2[0]*model_.fx(xm2,kdimen_));
  }
  delete [] xm1;
  delete [] xm2;
  return s *= xr;
