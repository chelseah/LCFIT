...
#include "model.h"
QuadSphere::QuadSphere(Model &model)
  : model_(model){
...
double QuadSphere::Integrate(double* xmin, int nmin, double* xmax, int nmax){
...
double QuadSphere::Integrate1D_(double x1, double x2){
  //x2>x1
  double xm = 0.5*(x1+x2);
  double xr = 0.5*(x2-x1);
  double s = 0;
...
double QuadSphere::Integrate2D_(double x1, double x2, double y){
...
double QuadSphere::Integrate3D_(double x1, double x2, double *y){
  //x2>x1, here x1 is y1, x2 is y2
  double xm = 0.5*(x1+x2);
  double xr = 0.5*(x2-x1);
  double s = 0;
  double *xm1 = new double [3];
  double *xm2 = new double [3];
  xm1[0] = y[0]; xm1[1] = y[1];
  xm2[0] = y[0]; xm2[1] = y[1];
  for (int j=0; j<5; j++){
    double dx = xr*absc_[j];
    xm1[2] = xm+dx;
    xm2[2] = xm-dx;
    s+=w_[j]*(xm1[0]*xm1[0]*sin(xm1[2])*model_.fx(xm1,kdimen_) + xm2[0]*xm2[0]*sin(xm2[2])*model_.fx(xm2,kdimen_));
  }
  delete [] xm1;
  delete [] xm2;
  return s *= xr;
